package com.liujc.maplib.utils;
...
import com.liujc.maplib.bean.AppInfo;
import com.liujc.maplib.bean.Location;

/**
 * 类名称：APPUtil
 * 创建者：Create by liujc
 * 创建时间：Create on 2017/4/5 13:43
 * 描述：APP工具类
 */
public class APPUtil {
	...
	public static void startNative_Baidu(Context context, Location loc1, Location loc2){
		...
		try {
			Intent intent = Intent.getIntent("intent://map/direction?origin=latlng:"+loc1.getStringLatLng()+"|name:"+loc1.getAddress()+"&destination=latlng:"+loc2.getStringLatLng()+"|name:"+loc2.getAddress()+"&mode=driving&src=androidExercise|cc#Intent;scheme=bdapp;package=com.baidu.BaiduMap;end");
	...
	public static void startNative_Gaode(Context context,Location loc){
		...
		try {
			Intent intent = new Intent("android.intent.action.VIEW",
					android.net.Uri.parse("androidamap://navi?sourceApplication=AndroidExercise&poiname=始发地&lat="+loc.getLat()+"&lon="+loc.getLng()+"&dev=1&style=2"));
